/**
 * 深证市场股票数据获取API - shenzheng.ts
 * 
 * =========================== 重构说明 ===========================
 * 
 * 📅 重构时间: 2025-01-27
 * 🎯 重构目的: 创建专门处理深圳证券交易所的独立API
 * 
 * 🏛️ 市场信息:
 * - 交易所: 深圳证券交易所 (SZSE)
 * - fs参数: m:0+t:6,m:0+t:80
 * - 输出文件: src/pages/data/astock/shenzheng.json
 * 
 * 📈 数据范围:
 * - 主板股票 (m:0+t:6)
 * - 创业板股票 (m:0+t:80)
...
const PAGE_SIZE = 100;
const TOTAL_PAGE_SIZE = 3100; // 深证股票数量预估
...
function buildUrl(page: number, pageSize: number): string {
  // 深证市场: m:0+t:6 (主板) + m:0+t:80 (创业板)
  const fsParam = "m%3A0%2Bt%3A6%2Cm%3A0%2Bt%3A80";
...
async function fetchAllStocks(): Promise<unknown[]> {
  ...
  console.log("🚀 [深证] fetchAllStocks ~ totalPages:", totalPages);
  ...
  for (const result of results) {
    if (result.data?.diff) {
      console.log(`[深证] 第${results.indexOf(result) + 1}页数据长度:`, result.data.diff.length);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // 写入到深证专用的JSON文件
    const filePath = path.join(dataDir, 'shenzheng.json');
    fs.writeFileSync(filePath, JSON.stringify(allStocks, null, 2), 'utf-8');
    
    console.log(`✅ 深证数据获取完成，共${allStocks.length}条记录`);
    
    return res.status(200).json({ 
      message: '深圳证券交易所股票数据获取并保存成功', 
      market: '深证',
      count: allStocks.length,
      filePath: 'src/pages/data/astock/shenzheng.json'
    });
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error('❌ 获取深证股票数据失败:', errorMessage);
