...
export const createEmailItem = (item: any, kltDesc: string | undefined, stockLink: string, stockName: string, suggestion: ERSISuggestion, backtestingStr: string = '') => {
    ...
    return `<tr><td>${item[0]}</td><td>${kltDesc}</td><td><a href="${stockLink}" style="color: ${stockColor};">${stockName}</a></td><td>${item[1]}</td><td style="color: ${buyColor};">${suggestion} ${backtestingStr}</td></tr>`;
};